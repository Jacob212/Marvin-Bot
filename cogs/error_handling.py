...
class errorHandlingCog(commands.Cog):
  def __init__(self,client):
    self.client = client
    self.generalList = self.client.get_cog("generalCommands").get_commands()#generalCommands cog must be loaded first
  ...
  async def on_command_error(self,ctx,error):
    ...
    if isinstance(error, commands.MissingRequiredArgument):
      ...
      for i in self.generalList:
        if str(error) == f'Command "{i.name}" is not found':
          await ctx.message.channel.send("**Command currently unavailable.**" + ctx.message.author.mention,delete_after=10)
          break
      else:
